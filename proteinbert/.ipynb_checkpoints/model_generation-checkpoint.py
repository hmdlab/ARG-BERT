...
from _new_tokenization import additional_token_to_index, n_tokens, tokenize_seq
#from _new_finetuning import mechanism_labels
...
class FinetuningModelGenerator(ModelGenerator):
    ...
    def __init__(self, pretraining_model_generator,  pretraining_model_manipulation_function = None, dropout_rate = 0.5, optimizer_class = None, \
            lr = None, other_optimizer_kwargs = None, model_weights = None, optimizer_weights = None):#output_spec,
    ...
    def create_model(self, seq_len, freeze_pretrained_layers = False):
        ...
        output_layer = keras.layers.Dense(6, activation = 'softmax')(last_hidden_layer)
...
def load_pretrained_model_from_dump(dump_file_path, create_model_function, create_model_kwargs = {}, optimizer_class = keras.optimizers.Adam, lr = 2e-04, \
    ...
    for m in model_weights:
        ave.append(np.average(m))
        std.append(np.std(m))
    print('平均：', sum(ave)/len(ave))
    print('分散：', sum(std)/len(std))
       
    
    # random weights
    np.random.seed(seed=4)
    rng = np.random.default_rng()
    model_weights =  [rng.normal(0.1012220101739613, 0.48538374483890445, w.shape) for w in model_weights]
    """
